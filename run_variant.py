...
VALID_TUNING_MODES = {"research", "oos_verify", "deploy", "production", "tuning"}

# Modes redefined 2026-05 (data-leakage-fix Task A step 1):
#   research   → enforce_oos_holdout=True (cutoff applied). Default for new variants.
#   oos_verify → enforce_oos_holdout=False; logs peek in experiment_inventory.json.
#                Use exactly once per candidate before promotion.
#   deploy     → enforce_oos_holdout=False; logs run in outputs/deploy_log.txt.
#                For the actually-deployed daily run (e.g. iter15_65tkr_reb21_vtg).
#   production → DEPRECATED alias for "research". Old manifests keep working.
#   tuning     → DEPRECATED alias for "research" (same semantics; old name).
_DEPRECATED_MODE_ALIAS = {"production": "research", "tuning": "research"}
...
def _git_hash() -> Optional[str]:
    """Local helper mirroring src.config._git_hash (avoid circular import)."""
    import subprocess
    from pathlib import Path as _P
    try:
        out = subprocess.check_output(
            ["git", "rev-parse", "HEAD"],
            cwd=_P(__file__).resolve().parent,
...
def _log_oos_peek(label: str, manifest_path: Path) -> None:
    """Append a peek record to experiment_inventory.json (atomic)."""
...
def _log_deploy(label: str) -> None:
    """Append a deploy-run record to outputs/deploy_log.txt."""
    from datetime import datetime
    log_path = Path("outputs") / "deploy_log.txt"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    line = (f"{datetime.utcnow().isoformat(timespec='seconds')}Z\t"
            f"{label}\t{_git_hash() or 'no-git'}\n")
...
def compose_config(manifest: Dict[str, Any]) -> PipelineConfig:
    """Compose PipelineConfig from manifest, applying tuning_mode semantics.

    Side effects:
      - oos_verify: appends to experiment_inventory.json (peek counter +=1).
      - deploy:     appends to outputs/deploy_log.txt.
      - production/tuning: emits DeprecationWarning, then proceeds as research.
    """
    import warnings
    overrides = dict(manifest.get("overrides") or {})
    tuning_mode = manifest.get("tuning_mode", "research")
    label = manifest.get("label", "<unlabelled>")
    manifest_path = manifest.get("_manifest_path", "<unknown>")

    # Deprecated alias handling
    if tuning_mode in _DEPRECATED_MODE_ALIAS:
        new_mode = _DEPRECATED_MODE_ALIAS[tuning_mode]
        warnings.warn(
            f"tuning_mode='{tuning_mode}' is DEPRECATED — mapping to '{new_mode}'. "
            "Use 'deploy' for the actually-deployed daily run, 'research' for "
            "exploratory variants (cutoff enforced), 'oos_verify' for the "
            "single post-cutoff peek before promotion.",
            DeprecationWarning, stacklevel=2,
        )
        tuning_mode = new_mode

    if tuning_mode == "research":
        overrides["enforce_oos_holdout"] = True
        # Default cutoff comes from DEFAULT_CONFIG (2024-12-31). Manifests may
        # override but cannot disable without using a different mode.
    elif tuning_mode == "oos_verify":
        overrides["enforce_oos_holdout"] = False
        _log_oos_peek(label, manifest_path)
    elif tuning_mode == "deploy":
        overrides["enforce_oos_holdout"] = False
        _log_deploy(label)
    else:
        raise ValueError(
            f"unknown tuning_mode: {tuning_mode!r}. Valid: {sorted(VALID_TUNING_MODES)}"
        )
...
def _summarize(metrics: Dict[str, Any], baseline_path: Path) -> None:
    ...
    print("=" * 60)
    # PRIMARY METRICS (selection-bias-discipline Task C step 3, 2026-05-19)
    if ir is not None:
        print(f"  IR              : {ir:.3f}")
    ri_mean = metrics.get("rolling_ir_mean")
    ri_min = metrics.get("rolling_ir_min")
    ri_pos = metrics.get("rolling_ir_pos_frac")
    if ri_mean is not None and ri_mean == ri_mean:
        print(f"  Rolling IR (252d): mean={ri_mean:.3f}, min={ri_min:.3f}, pos_frac={ri_pos:.2f}")
    ...
    print(f"  Turnover (2-way): {metrics.get('avg_annual_turnover', 0.0) * 100:.1f}%")
    ...
    # OOS peek counter (data-leakage-fix Task A step 1)
    inv_path = Path("experiment_inventory.json")
    if inv_path.exists():
        try:
            inv = json.loads(inv_path.read_text(encoding="utf-8"))
            n_peeks = inv.get("n_oos_peeks", 0)
            print(f"  OOS peeks (n_oos_peeks in experiment_inventory.json) : {n_peeks}")
        except Exception:
            pass
...
def run(manifest_path: Path, no_cache: bool = False) -> int:
    ...
    SAFE_FOR_CACHE_REUSE = frozenset({
        ...
        "rebalance_freq", "one_way_tc", "subperiod_breakpoints",
        ...
        "max_active_share", "sector_deviation", "bm_weight_floor",
        "cov_lookback",
        # Portfolio style (optimizer-level)
        "portfolio_style", "satellite_budget", "satellite_max_per_stock",
        "core_lambda_multiplier", "enforce_score_gated_ow",
        "score_threshold_for_ow",
        ...
        "mega_cap_protection_enabled", "mega_cap_funding_mode",
        "mega_cap_funding_k", "mega_cap_uw_active_max",
        "mega_cap_ow_active_max", "mega_cap_threshold",
        "bm_proportional_cap_enabled", "bm_proportional_cap_min",
        "bm_proportional_cap_max",
        ...
        "growth_tilt_revision_share", "growth_tilt_eps_share",
        "growth_tilt_horizon_days",
        "pead_boost_enabled", "pead_boost_weight",
        "pead_boost_decay_days", "pead_boost_cutoff_days",
        "signal_stability_lambda", "signal_stability_decay",
    })
    overrides = manifest.get("overrides") or {}
    unsafe_keys = sorted(set(overrides.keys()) - SAFE_FOR_CACHE_REUSE)
